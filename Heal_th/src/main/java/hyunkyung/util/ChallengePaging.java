...
//		
		...
		if(listCount == 0)	this.listCount = 6; //화면에 보여질 게시글 수를 6개로 설정
		if(pageCount == 0)	this.pageCount = 6; //화면에 보여질 페이징 수를 6개로 설정
		
//	      //기본값 설정
//	      if(curPage == 0)   setCurPage(1); //URL에 페이지 번호가 없지만 첫 페이지를 기본 페이지로 설정한다
//	      if(listCount == 0)   setListCount(6);   //화면에 보여질 게시글 수를 6개로 기본 설정한다
//	      if(pageCount == 0)   setPageCount(6);   //화면에 보여질 페이지 수를 6개로 기본 설정한다
